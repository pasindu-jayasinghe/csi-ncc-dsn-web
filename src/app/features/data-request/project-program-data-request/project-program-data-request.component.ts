...
@Component({
  selector: 'app-project-program-data-request',
  templateUrl: './project-program-data-request.component.html',
  styleUrls: ['./project-program-data-request.component.scss']
})
export class ProjectProgramDataRequestComponent implements OnInit {
  ...
  constructor(private serviceProxy: ServiceProxy, private subServiceProxy: ProjectProgramDataControllerServiceProxy, private masterDataControllerServiceProxy: MasterdataControllerServiceProxy) { }
  ...
  search = () => {
    ...
    this.loadProjectPrograms(event);
  ...
  submitRequests = () => {
    ...
    this.serviceProxy.createManyBaseProjectProgramDataControllerProjectProgramData(list).subscribe(res => {
      if (res && res.length > 0) {
        ...
        this.deadline = null; // reset deadline
  ...
  onShowCancel() {
    if (this.selectedRequestedProjectPrograms != null && this.selectedRequestedProjectPrograms != undefined && this.selectedRequestedProjectPrograms.length > 0) {
      ...
      this.DisplayAlert('Please select at least one project/program.', AlertType.Warning);
  ...
  async updateStatus() {
    ...
    if (this.selectedRequestedProjectPrograms != null && this.selectedRequestedProjectPrograms != undefined && this.selectedRequestedProjectPrograms.length > 0) {
      ...
      if (this.isCancel) {
        await this.subServiceProxy.cancelProjectDataFromCCS(request).subscribe(res => {
          console.log(res);
          if (res) {
            this.DisplayAlert('Successfully Cancel.', AlertType.Message);
            this.showRejectPop = false;
            this.rejectComment = "";
            this.selectedRequestedProjectPrograms = new Array();
            this.search();
